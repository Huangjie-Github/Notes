...
思路： 首先按照歌手的A-Z的顺序，找出一个接口，能够获取所有的歌手信息的，然后通过找到的歌手信息，获取该歌手的所有发布歌曲，通过找到的歌曲列表获取单个歌曲的信息，通过歌曲信息，找到歌曲的播放接口，也就是返回bit字节的接口，然后把拿到bit字节写入本地文件，至此结束。

获取sign校验码的js文件，`sing.js`
...
返回请求接口的`python`文件，`base_util.py`
...
def get_sign(data):
    with open('./sign.js', mode='r', encoding='utf-8') as open_js:
        sing_js = execjs.compile(open_js.read())
        return sing_js.call('getSign', data)


def get_singer_data(index, cur_page):
    return '{"comm":{"ct":24,"cv":0},"singerList":{"module":"Music.SingerListServer","method":"get_singer_list",' \
           '"param":{"area":-100,"sex":-100,"genre":-100,"index":%s,"sin":0,"cur_page":%s}}} ' % (
               str(index), str(cur_page))


def get_singer_url(sign, data):
    return 'https://u.y.qq.com/cgi-bin/musics.fcg?-=getUCGI054145656769473804&g_tk=434384553&sign' \
           '=%s&loginUin=1728537059&hostUin=0&format=json&inCharset' \
           '=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0&data=%s' % (str(sign), str(data))


def get_singer_music_data(singer_mid, num):
    return '{"comm":{"ct":24,"cv":0},"singerSongList":{"method":"GetSingerSongList","param":{"order":1,' \
           '"singerMid":"%s","begin":0,"num":%s},"module":"musichall.song_list_server"}} ' % (str(singer_mid), str(num))


def get_singer_music_url(sign, data):
    return 'https://u.y.qq.com/cgi-bin/musics.fcg?-=getSingerSong615958749210471&g_tk=434384553&sign' \
           '=%s&loginUin=1728537059&hostUin=0&format=json&inCharset' \
           '=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0&data=%s' % (str(sign), str(data))


def get_music_song_data(song_mid):
    return '{"req":{"module":"CDN.SrfCdnDispatchServer","method":"GetCdnDispatch","param":{"guid":"4179379842",' \
           '"calltype":0,"userip":""}},"req_0":{"module":"vkey.GetVkeyServer","method":"CgiGetVkey",' \
           '"param":{"guid":"4179379842","songmid":["%s"],"songtype":[0],"uin":"1728537059",' \
           '"loginflag":1,"platform":"20"}},"comm":{"uin":1728537059,"format":"json","ct":24,"cv":0}} ' % (
               str(song_mid))


def get_music_song_url(sign, data):
    return 'https://u.y.qq.com/cgi-bin/musics.fcg?-=getplaysongvkey5938397311173371&g_tk=434384553&sign' \
           '=%s&loginUin=1728537059&hostUin=0&format=json&inCharset=utf8' \
           '&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0&data=%s' % (str(sign), str(data))

```

执行爬取Music的`pyhton`文件，`__init__.py`
...
import math
import requests
from python import base_util
from concurrent.futures import ThreadPoolExecutor

thread_bus = ThreadPoolExecutor(max_workers=10)
folder_name = './QQMusic/'


def download_music(music_purl, singer_name, sing_name):
    url = 'http://ws.stream.qqmusic.qq.com/' + music_purl
    resp = requests.get(url)
    resp.encoding = 'utf-8'
    # print('------------------%s-----------------' % resp.status_code)
    if resp.status_code is 200:
        file_name = "{}--{}.m4a".format(singer_name, sing_name)
        with open(folder_name + file_name, mode='wb') as music_file:
...
def search_music_song(song_mid, singer_name, sing_name):
    data = base_util.get_music_song_data(song_mid)
    sign = base_util.get_sign(data)
    url = base_util.get_music_song_url(sign, data)
    resp = requests.get(url).json()
    music_purl = resp['req_0']['data']['midurlinfo'][0]['purl']
    # 传输音乐的加密信息，调用下载方法
    download_music(music_purl, singer_name, sing_name)


def search_singer_music(singer_mid, singer_name):
    data = base_util.get_singer_music_data(singer_mid, 1)
    sign = base_util.get_sign(data)
    url = base_util.get_singer_music_url(sign, data)
    resp = requests.get(url).json()
    total_num = resp['singerSongList']['data']['totalNum']
    # 二层请求
    data = base_util.get_singer_music_data(singer_mid, total_num)
    sign = base_util.get_sign(data)
    url = base_util.get_singer_music_url(sign, data)
    resp = requests.get(url).json()
    music_list = resp['singerSongList']['data']['songList']
    for music in music_list:
        sing_mid = music['songInfo']['mid']
        sing_name = music['songInfo']['name']
        search_music_song(sing_mid, singer_name, sing_name)


# 获得字母对应歌手
def search_singer(index):
    data = base_util.get_singer_data(index, 1)
    sign = base_util.get_sign(data)
    url = base_util.get_singer_url(sign, data)
    resp = requests.get(url).json()
    # 获取歌手的总数，然后除以每页的数目，得到页数，因为接口固定一页8
    pages = math.ceil(resp['singerList']['data']['total'] / 80)

    for cur_page in range(pages):
        data = base_util.get_singer_data(index, cur_page)
        sign = base_util.get_sign(data)
        url = base_util.get_singer_url(sign, data)
        resp = requests.get(url).json()
        singer_list = resp['singerList']['data']['singerlist']
        for singer in singer_list:
            singer_mid = singer['singer_mid']
            singer_name = singer['singer_name']
            # 多线程下载地方
            # thread_bus.submit(search_singer_music, singer_mid=singer_mid, singer_name=singer_name)
            search_singer_music(singer_mid, singer_name)
